...
def _render_cruzamento_saidas(escolhido: dict) -> None:
    ...
    **Busca3** (loader.cruzar_produto_escolhido_saidas_criterio3()) —
    ...
    Descrição EFETIVA (2026-08-04): ver docstring de _render_
    cruzamento_entradas() pro raciocínio completo — mesmo
...
def render_cruzamento_produtos() -> None:
    ...
    if not escolhido_atual:
        ...
        with st.container(key=chave_header):
            ...
            # final de cada passo (10 critérios + 3 origens de FM) — usado
            # aqui pra avançar a barra em tempo real, mesmo padrão de
            # `_barra_progresso()` já usado na tela de Extração.
            #
            # 2026-08-20: chegou a ter um "📋 Painel de Revisão" entre a
            # confirmação e a aplicação de FM (rascunho em cruzamento_
            # provisorio/_detalhado, homologação manual) — REVERTIDO no
            # mesmo dia, pedido do usuário ("desfazer a revisão
            # obrigatória"): volta ao fluxo direto de sempre, confirma E
            # aplica FM no mesmo clique, sem passar por rascunho.
