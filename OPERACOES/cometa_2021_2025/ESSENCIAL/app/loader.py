...
def salvar_grupo_produto_alvo_fiscalizacao(selecoes: pd.DataFrame) -> dict:
    ...
    TOTAL_DEBITO, TOTAL_CREDITO, OBSERVACAO.
    ...
    try:
        ...
        if not existente.empty and "DESCR_EDITADA" in existente.columns:
            descr_editada_existente = _mapa_por_chave_produto_alvo_fiscalizacao(existente, "DESCR_EDITADA")
            novo["DESCR_EDITADA"] = chave_novo.map(descr_editada_existente).fillna("")
        else:
            novo["DESCR_EDITADA"] = ""
...
def salvar_edicoes_produto_alvo_salvos(atualizacoes: pd.DataFrame) -> dict:
    """Persiste os campos editáveis do Estágio 10 — "PRODUTOS ALVOS
    ...
    try:
        ...
        logger.exception("Erro ao salvar edições em produto_alvo_fiscalizacao: %s", exc)
        resultado["erro"] = str(exc)
    return resultado
...
def _codigos_por_ocorrencia_733(ocorrencias_concat: str) -> "list[str]":
    """Lista de códigos DISTINTOS extraída de uma string com códigos
    concatenados por ", " SEM distinct — um código por item físico real
    (ver `COD_ITEM_OCORRENCIAS`, `_QUERY_CONSOLIDADO_733_XML`/`_ESTOQUE`
    em `gerar_consolidado_origens_733()`) — ORDENADA por OCORRÊNCIA
    DECRESCENTE (o código mais frequente vem PRIMEIRO). Empate resolvido
    pela ORDEM de aparição (`Counter.most_common()` é estável: entre 2
    códigos com a MESMA contagem, vence o que apareceu primeiro na
    string). `[]` se `ocorrencias_concat` vier vazio/None."""
    ...
    if not codigos:
        return []
    return [codigo for codigo, _contagem in Counter(codigos).most_common()]


def _codigo_mais_frequente_733(ocorrencias_concat: str) -> str:
    """Extrai só o PRIMEIRO código de `_codigos_por_ocorrencia_733()` —
    o de MAIOR OCORRÊNCIA. Pedido do usuário (2026-08-16): "nas
    entradas, saídas, estoque, em caso de mais de um cod prod, cravar o
    cód de maior ocorrência". `""` se não houver nenhum código."""
    lista = _codigos_por_ocorrencia_733(ocorrencias_concat)
...
def unificar_por_produto_733(df: pd.DataFrame) -> pd.DataFrame:
    ...
    `COD_ITEM` (exibição, coluna "Cód. Produto" na grade) reagregado com
    os códigos DISTINTOS de todos os anos, ORDENADOS por OCORRÊNCIA
    DECRESCENTE — o mais frequente aparece PRIMEIRO na lista (2026-08-16,
    ...
    ser o de MAIOR OCORRÊNCIA entre os itens físicos reais — mesmo
    critério de `COD_ITEM` acima, mas só o PRIMEIRO da lista.

    Ambos calculados a partir de `COD_ITEM_OCORRENCIAS` (concatenação
    SEM distinct — um código por item físico real, ver `gerar_
    consolidado_origens_733()`/`_QUERY_CONSOLIDADO_733_XML`), reagregada
    aqui por simples concatenação de string entre os anos (preserva a
    contagem de cada código) e resolvida por `_codigos_por_ocorrencia_
    733()` (`collections.Counter`, empate resolvido pela ORDEM de
    aparição). Contagem sempre escopada DENTRO do mesmo `DESCR_PROD` —
    nunca mistura ocorrências de produtos diferentes, mesmo que
    compartilhem um código (achado real: código `00000000024045`
    aparece tanto em "#DES.ABOVE AER.MEN E.CARB.150M" quanto em
    "#DES.ABOVE AER.WOM.T.SWE.150ML", cada um com sua própria contagem,
    confirmado 2026-08-16). Cache SEM `COD_ITEM_OCORRENCIAS` (gerada
    antes de 2026-08-16) cai pro `COD_ITEM` alfabético antigo e não
    ganha `COD_ITEM_FREQUENTE` — precisa de "Regerar Consolidado".
    ...
    if "COD_ITEM_OCORRENCIAS" in unificado.columns:
        listas_por_ocorrencia = unificado["COD_ITEM_OCORRENCIAS"].apply(_codigos_por_ocorrencia_733)
        unificado["COD_ITEM_FREQUENTE"] = listas_por_ocorrencia.apply(lambda lst: lst[0] if lst else "")
        # COD_ITEM (exibição) reordenado por ocorrência decrescente — substitui
        # a versão alfabética de _concat_cod_item_distinto() acima quando há
        # dado de ocorrência disponível (pedido do usuário, 2026-08-16).
        if "COD_ITEM" in unificado.columns:
            unificado["COD_ITEM"] = [
                ", ".join(lst) if lst else _MARCADOR_COD_ITEM_AUSENTE_733 for lst in listas_por_ocorrencia
            ]
    return unificado
...
#
# Estendido pra Saídas/Estoque (2026-07-26, pedido do usuário: "ESTENDA
# PARA SAIDAS E ESTOQUES") — 1 tabela por origem (mesmo padrão de
# fm_entradas_curadoria/fm_saidas_curadoria/fm_estoque_curadoria do
...
_TABELAS_TRATAMENTO_FM = {
    ...
    "saidas": "tratamento_fm_saidas",
    "estoque": "tratamento_fm_estoque",
}


def consultar_tratamento_fm_por_idunico(idunicos: "set | list", origem: str = "entradas") -> pd.DataFrame:
    ...
    try:
        ...
        with duckdb.connect(str(_BANCO_PATH)) as con:
            ...
            if tabela in tabelas:
                ...
                combinado = novo
            combinado = combinado[_COLUNAS_TRATAMENTO_FM].reset_index(drop=True)
        ...
        logger.exception("Erro ao aplicar %s em %s", tabela, _BANCO_PATH)
        return {"erro": str(e)}
...
# Produto Divergente".
# Rótulo (2026-07-28, pedido do usuário: "mudar o nome do critério3
# para: 'nome_prod_decl do alvo = nome_prod_xml' em entradas") — mesma
# comparação de sempre (SIMILARIDADE_DESCRICAO entre desc_xml do
# candidato e DESCR_ALVO), só o RÓTULO ficou mais descritivo (segue o
# padrão "X do alvo = Y do candidato" de CRITERIO_BUSCA1/2). Mesmo dia,
# aplicado a Saídas também ("nas saídas transforme busca3 em 2 e mude
# o texto conforme entradas") — chegou a ser renumerado pra "Busca2"
# lá (Saídas só tem 2 critérios), mas revertido no mesmo dia ("vamos
# fixar. volte para o nr 3.") — Entradas e Saídas usam a MESMA
...
_COLUNAS_CRUZAMENTO_FINAL_PRODUTO = [
    ...
    "PU_SUGERIDO", "BASE_CALCULO", "ICMS", "MULTA", "CREDITO_TRIBUTARIO", "CONDICAO_PU", "AGREGACAO",
...
def _calcular_campos_derivados_cruzamento_final(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ICMS/MULTA/CREDITO_TRIBUTARIO/CONDICAO_PU/AGREGACAO a partir de
    QTDE_EI/QTDE_C/QTDE_V/QTDE_EF/MEDIA_PU_C/MEDIA_PU_V/ALIQ já
    presentes em `df` (essas 7 colunas nunca faltam — são a base
    original do Estágio 10.2, de 2026-08-05) — extraída em 2026-08-06
    pra ser reaproveitada em 2 pontos: gerar_cruzamento_final_produto()
    (cálculo novo, produto escolhido) e consultar_cruzamento_final_
    produto() (migração de schema em leitura — linhas persistidas ANTES
    de TD/TC/etc. existirem, mesmo dia, não têm essas colunas na tabela
    real). Mesma regra RN1 documentada em `regra de negócios
    unificadas/regra negocio_pu_rn1_ei+c=v+ef_1.txt` — ver docstring de
    gerar_cruzamento_final_produto() pra tabela completa dos 4
    sub-cenários de PU e da Liquidação Tributária (ICMS/MULTA/CREDITO_
    TRIBUTARIO). Devolve CÓPIA de `df` com as 11 colunas novas
    adicionadas/sobrescritas."""
    df = df.copy()
    ...
    # Liquidação Tributária (2026-08-06, Solicitação Técnica "LIQUIDAÇÃO
    # TRIBUTÁRIA DO CRUZAMENTO") — encadeamento BASE_CALCULO → ICMS →
    # MULTA → CREDITO_TRIBUTARIO, mesmo raciocínio de fallback NaN->0.0
    # (pedido explícito: "trate valores nulos ... convertendo-os para
    # 0.0"), embora BASE_CALCULO/ALIQ já não venham NaN.
    df["ICMS"] = (df["BASE_CALCULO"] * (df["ALIQ"] / 100)).fillna(0.0)
    df["MULTA"] = (df["ICMS"] * 0.75).fillna(0.0)
    df["CREDITO_TRIBUTARIO"] = (df["ICMS"] + df["MULTA"]).fillna(0.0)
...
def gerar_cruzamento_final_produto(escolhido: dict) -> pd.DataFrame:
    ...
    - BASE_CALCULO (2026-08-06, Solicitação Técnica "PERSISTÊNCIA
      AUTOMÁTICA E VALORAÇÃO DO RISCO") = PU_SUGERIDO × DIF_QTDE — valor
      em R$ da omissão daquele ano/produto. Zero quando TD==TC (sem
      infração, DIF_QTDE=0) — nunca `NaN` (PU_SUGERIDO/DIF_QTDE já vêm
      tratados).
    - ICMS/MULTA/CREDITO_TRIBUTARIO (2026-08-06, Solicitação Técnica
      "LIQUIDAÇÃO TRIBUTÁRIA DO CRUZAMENTO") — fecha a camada financeira
      do Estágio 10.2, encadeando a partir de BASE_CALCULO:
      - ICMS = BASE_CALCULO × (ALIQ / 100) — aplica a alíquota do ano
        (18%/20%, ver ALIQ acima) sobre o valor da omissão.
      - MULTA = ICMS × 0,75 — penalidade de 75% sobre o imposto apurado
        (pedido explícito, sem confirmação extra necessária — regra
        numérica direta do usuário).
      - CREDITO_TRIBUTARIO = ICMS + MULTA — risco financeiro TOTAL da
        linha (base pro futuro Auto de Infração, Estágio 15). Zero
        quando BASE_CALCULO=0 (sem infração) — nunca `NaN`.
...
def salvar_cruzamento_final_produto(escolhido: dict, editado: pd.DataFrame) -> dict:
    ...
    QTDE_EI/QTDE_C/TD/QTDE_V/QTDE_EF/TC/INFRACAO_FINAL/DIF_QTDE/
    PU_SUGERIDO/BASE_CALCULO/ICMS/MULTA/CREDITO_TRIBUTARIO/CONDICAO_PU/
    AGREGACAO/MEDIA_PU_C/MEDIA_PU_V/MEDIA_PU_E) — TD/TC/INFRACAO_FINAL/
    DIF_QTDE/PU_SUGERIDO/BASE_CALCULO/ICMS/MULTA/CREDITO_TRIBUTARIO/
    CONDICAO_PU/AGREGACAO (2026-08-06) são campos CALCULADOS por
    gerar_cruzamento_final_
    ...
    outro meio, ou forçar um PU/condição/ICMS/multa diferente) — salvos
    exatamente
...
def consultar_cruzamento_final_produto(
    ...
    INFRACAO_FINAL/DIF_QTDE/PU_SUGERIDO/BASE_CALCULO/ICMS/MULTA/
    CREDITO_TRIBUTARIO/CONDICAO_PU/AGREGACAO existirem — não têm essas
    colunas na tabela DuckDB real; `SELECT *` só traz o que existe de
    verdade). `UP` (2026-08-05) completado com `""` se ausente (produto
    salvo antes até dela existir); os 11 campos derivados da regra RN1/
    Liquidação Tributária, se QUALQUER um estiver ausente, são
    RECALCULADOS via `_calcular_campos_derivados_cruzamento_final()` a
    partir de QTDE_EI/QTDE_C/QTDE_V/QTDE_EF/MEDIA_PU_C/MEDIA_PU_V/ALIQ
    (essas 7 SEMPRE existem — são a base original do Estágio 10.2) —
    sem precisar reprocessar o banco nem re-salvar nada. Crítico pra
    `salvar_cruzamento_final_produto()`: sem essa migração, o upsert
    preservaria linhas de OUTROS produtos com TD/TC/etc. faltando,
    virando `NaN` no `pd.concat()` com as linhas novas (que já têm as
    11 colunas)."""
    ...
    try:
        ...
        campos_derivados_rn1 = {
            "TD", "TC", "INFRACAO_FINAL", "DIF_QTDE", "PU_SUGERIDO", "BASE_CALCULO",
