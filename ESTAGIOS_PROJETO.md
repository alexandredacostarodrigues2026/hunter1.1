...
| 7.3 | RN1 — Movimentação Física (XML) — mesma identidade EI+Compras=Vendas+EF do 7.2 (Compras/Vendas já vinham do XML, Estágio 4; EI/EF da declaração, Estágio 5), agregada por (Ano, Descrição Relevante) em vez de (Ano, COD_ITEM) — a Descrição Relevante (7.1) vira o elo direto entre movimentação e inventário | ✅ Implementado (`rn1_fisica`; botão de 9º nível próprio no Menu Principal — "🔥 7.3: RN1 — MOVIMENTAÇÃO FÍSICA (XML)"; reaproveita `cruzamento_valor` já persistida, mesma técnica de condensação do 7.2.1, mas mantendo o Ano separado; **não é o Estágio 15** — sem a lógica de PU (preço unitário)/tie-break do texto original da RN1, que continua reservada; itens de entrada sem match no BC3 continuam fora de Compras, mesma limitação do 7.2/produto_alvo; testado nas 3 operações reais — geraldo R$1.228.880,31 de divergência acumulada/5.721 produtos, PB2 R$5.072.425,40/249, cometa R$397.552.932,21/2.004) | sem doc própria ainda; ver `interface.render_pagina_rn1_fisica()`/`loader.gerar_rn1_fisica()` |
