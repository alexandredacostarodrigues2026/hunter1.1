...
| 7.3 | RN1 — Movimentação Física (XML) — mesma identidade EI+Compras=Vendas+EF do 7.2, mas Compras soma o valor TOTAL do XML de entradas (Estágio 4), inclusive itens sem match no Matching/BC3 (o 7.2 só soma itens COM match) — agregada por (Ano, Descrição Relevante) em vez de (Ano, COD_ITEM), Descrição Relevante (7.1) como elo entre movimentação e inventário | ✅ Implementado (`rn1_fisica`; botão de 9º nível próprio no Menu Principal — "🔥 7.3: RN1 — MOVIMENTAÇÃO FÍSICA (XML)"; Compras via nova `loader._compras_entradas_sem_filtro_bc3()` (sem o filtro `COD_ITEM_DECLARACAO IS NOT NULL` do 7.2), itens sem match nenhum somam numa linha sentinela `"(SEM VÍNCULO NO MATCHING)"` em vez de sumirem; Vendas/EI/EF reaproveitados de `cruzamento_valor` (sem o mesmo problema de cobertura); **não é o Estágio 15** — sem a lógica de PU (preço unitário)/tie-break do texto original da RN1, que continua reservada; testado nas 3 operações reais — geraldo R$1.230.187,38 de divergência acumulada/R$1.307,07 sem vínculo, PB2 R$5.412.013,80/R$339.992,00 sem vínculo, cometa R$323.652.947,55/R$2.252.713,86 sem vínculo) | sem doc própria ainda; ver `interface.render_pagina_rn1_fisica()`/`loader.gerar_rn1_fisica()` |
