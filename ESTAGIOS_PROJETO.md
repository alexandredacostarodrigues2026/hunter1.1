...
| 7 | Descrição Relevante — elege a descrição mais frequente (moda) por COD_ITEM entre estoque_entradas, estoque_saidas (Estágio 4) e estoque_anual_consolidado (Estágio 5), padronizando o nome "oficial" do produto pra relatórios e seleção de itens pra auditoria física | ✅ Implementado (`produto_alvo`; botão de 6º nível próprio no Menu Principal — "🏷️ DESCRIÇÃO RELEVANTE" — em vez de sub-painel dentro de "Tabelas Entradas/Saídas/Estoques" como a nota de numeração original previa; testado nas 3 operações reais: geraldo 6.138 produtos, PB2 269, cometa 2.617) | sem doc própria ainda; ver `interface.render_pagina_descricao_relevante()`/`loader.montar_produto_alvo()` |
...
- Numeração não é estritamente sequencial (ver o salto 6 → 15): o Estágio 6
  ...
  renumerar de novo. **Atualizado 2026-07-18**: o Estágio 7 (Descrição
  Relevante) usou esse espaço reservado, mas — por Solicitação Técnica
  explícita — virou botão de 6º nível PRÓPRIO no Menu Principal, não um
  sub-painel dentro de "Tabelas Entradas / Saídas / Estoques" como a
  reserva original previa; os números 8-14 continuam livres pra painéis
  futuros, do mesmo jeito (sub-painel ou botão próprio, a decidir caso a
  caso).
